...
class JointKLDivergenceAnalyzer:
    ...
    def calculate_cross_entropy(self, original_logits, modified_logits):
        """
        Calculate cross-entropy between original and modified next-token predictions.
        ...
        Returns:
            cross_entropies: Cross-entropy for each position [batch, seq]
        """
        # Get original model's predicted token (ground truth for this comparison)
        original_targets = torch.argmax(original_logits, dim=-1)  # [batch, seq]
        
        # Calculate cross-entropy loss of modified model predicting original targets
        cross_entropy = F.cross_entropy(
            modified_logits.view(-1, modified_logits.size(-1)),  # [batch*seq, vocab]
            original_targets.view(-1),  # [batch*seq]
            reduction='none'
        ).view(original_targets.shape)  # [batch, seq]
    ...
    def run_joint_kl_comparison_experiment(self, wiki_texts, num_epsilon_samples_per_text=3, batch_size=4):
        ...
        for batch_start in tqdm(range(0, len(wiki_texts), batch_size), desc="Processing wiki batches"):
            ...
            try:
                ...
                # Calculate KL divergences and cross-entropies for SAE and Joint
                ...
                ce_sae = self.calculate_cross_entropy(original_logits, sae_logits)
                ce_joint = self.calculate_cross_entropy(original_logits, joint_logits)
                ...
                for sample_idx in range(num_epsilon_samples_per_text):
                    ...
                    ce_epsilon = self.calculate_cross_entropy(original_logits, epsilon_logits)
                    kl_epsilon_samples.append(kl_epsilon)
                    ce_epsilon_samples.append(ce_epsilon)
                ...
                for batch_idx in range(batch_size_actual):
                    ...
                    for pos_idx in range(min(actual_seq_len, seq_len)):
                        ...
                        results.append({
                            ...
                            'cross_entropy': float(ce_sae[batch_idx, pos_idx].cpu()),
                            'sample_idx': 0,
                        ...
                        results.append({
                            ...
                            'cross_entropy': float(ce_joint[batch_idx, pos_idx].cpu()),
                            'sample_idx': 0,
                            'epsilon_distance': None
                        })
    ...
    def analyze_joint_kl_results(self, results_df):
        ...
        # Calculate summary statistics by intervention type for both metrics
        kl_summary_stats = results_df.groupby('intervention_type')['kl_divergence'].agg([
            'count', 'mean', 'std', 'median', 'min', 'max'
        ]).round(6)
        
        ce_summary_stats = results_df.groupby('intervention_type')['cross_entropy'].agg([
            'count', 'mean', 'std', 'median', 'min', 'max'
        ]).round(6)
        
        print(f"\nKL Divergence Summary Statistics:")
        print(kl_summary_stats)
        print(f"\nCross-Entropy Summary Statistics:")
        print(ce_summary_stats)
        
        # Calculate ratios for both metrics
        epsilon_kl_mean = kl_summary_stats.loc['epsilon_random', 'mean']
        sae_kl_mean = kl_summary_stats.loc['SAE_only', 'mean'] 
        joint_kl_mean = kl_summary_stats.loc['Joint_SAE_NFM', 'mean']
        
        epsilon_ce_mean = ce_summary_stats.loc['epsilon_random', 'mean']
        sae_ce_mean = ce_summary_stats.loc['SAE_only', 'mean'] 
        joint_ce_mean = ce_summary_stats.loc['Joint_SAE_NFM', 'mean']
        
        sae_vs_epsilon_kl_ratio = sae_kl_mean / epsilon_kl_mean
        joint_vs_epsilon_kl_ratio = joint_kl_mean / epsilon_kl_mean
        joint_vs_sae_kl_ratio = joint_kl_mean / sae_kl_mean
        
        sae_vs_epsilon_ce_ratio = sae_ce_mean / epsilon_ce_mean
        joint_vs_epsilon_ce_ratio = joint_ce_mean / epsilon_ce_mean
        joint_vs_sae_ce_ratio = joint_ce_mean / sae_ce_mean
        
        print(f"\nKL Divergence Ratios:")
        print(f"  SAE vs ε-random: {sae_vs_epsilon_kl_ratio:.3f}x")
        print(f"  Joint vs ε-random: {joint_vs_epsilon_kl_ratio:.3f}x") 
        print(f"  Joint vs SAE: {joint_vs_sae_kl_ratio:.3f}x")
        
        print(f"\nCross-Entropy Ratios:")
        print(f"  SAE vs ε-random: {sae_vs_epsilon_ce_ratio:.3f}x")
        print(f"  Joint vs ε-random: {joint_vs_epsilon_ce_ratio:.3f}x") 
        print(f"  Joint vs SAE: {joint_vs_sae_ce_ratio:.3f}x")
        ...
        joint_ces = results_df[results_df['intervention_type'] == 'Joint_SAE_NFM']['cross_entropy']
        
        # T-tests for KL divergence
        sae_vs_epsilon_kl_stat, sae_vs_epsilon_kl_p = stats.ttest_ind(sae_kls, epsilon_kls)
        joint_vs_epsilon_kl_stat, joint_vs_epsilon_kl_p = stats.ttest_ind(joint_kls, epsilon_kls)
        joint_vs_sae_kl_stat, joint_vs_sae_kl_p = stats.ttest_ind(joint_kls, sae_kls)
        
        # T-tests for Cross-entropy
        sae_vs_epsilon_ce_stat, sae_vs_epsilon_ce_p = stats.ttest_ind(sae_ces, epsilon_ces)
        joint_vs_epsilon_ce_stat, joint_vs_epsilon_ce_p = stats.ttest_ind(joint_ces, epsilon_ces)
        joint_vs_sae_ce_stat, joint_vs_sae_ce_p = stats.ttest_ind(joint_ces, sae_ces)
        
        print(f"\nKL Divergence Statistical Tests (t-tests):")
        print(f"  SAE vs ε-random: t={sae_vs_epsilon_kl_stat:.3f}, p={sae_vs_epsilon_kl_p:.6f}")
        print(f"  Joint vs ε-random: t={joint_vs_epsilon_kl_stat:.3f}, p={joint_vs_epsilon_kl_p:.6f}")
        print(f"  Joint vs SAE: t={joint_vs_sae_kl_stat:.3f}, p={joint_vs_sae_kl_p:.6f}")
        
        print(f"\nCross-Entropy Statistical Tests (t-tests):")
        print(f"  SAE vs ε-random: t={sae_vs_epsilon_ce_stat:.3f}, p={sae_vs_epsilon_ce_p:.6f}")
        print(f"  Joint vs ε-random: t={joint_vs_epsilon_ce_stat:.3f}, p={joint_vs_epsilon_ce_p:.6f}")
        print(f"  Joint vs SAE: t={joint_vs_sae_ce_stat:.3f}, p={joint_vs_sae_ce_p:.6f}")
        
        # Interpretation (primarily based on KL divergence, with cross-entropy as supporting evidence)
        print(f"\n=== HYPOTHESIS TESTING (JOINT ARCHITECTURE SCALE) ===")
        
        # Gurnee's pathological error hypothesis (KL-based)
        if sae_vs_epsilon_kl_ratio > 1.5 and sae_vs_epsilon_kl_p < 0.05:
            print(f"✓ GURNEE'S PATHOLOGICAL ERROR CONFIRMED: SAE errors are {sae_vs_epsilon_kl_ratio:.1f}x worse than random (KL)")
            print(f"  Cross-entropy supports: {sae_vs_epsilon_ce_ratio:.1f}x worse than random (CE)")
        else:
            print(f"✗ Gurnee's pathological error NOT confirmed")
        
        # Joint end-to-end training hypothesis (KL-based)
        if joint_vs_sae_kl_ratio < 0.8 and joint_vs_sae_kl_p < 0.05:
            print(f"✓ JOINT END-TO-END TRAINING HYPOTHESIS STRONGLY SUPPORTED: Joint reduces KL by {(1-joint_vs_sae_kl_ratio)*100:.1f}%")
            print(f"  Cross-entropy supports: Joint reduces CE by {(1-joint_vs_sae_ce_ratio)*100:.1f}%")
        elif joint_vs_sae_kl_ratio < 1.0 and joint_vs_sae_kl_p < 0.05:
            print(f"✓ JOINT END-TO-END TRAINING HYPOTHESIS SUPPORTED: Joint reduces KL by {(1-joint_vs_sae_kl_ratio)*100:.1f}%")
            print(f"  Cross-entropy supports: Joint reduces CE by {(1-joint_vs_sae_ce_ratio)*100:.1f}%")
        elif joint_vs_sae_kl_ratio < 1.0:
            print(f"~ JOINT END-TO-END TRAINING HYPOTHESIS PARTIALLY SUPPORTED: Joint reduces KL by {(1-joint_vs_sae_kl_ratio)*100:.1f}% (not significant)")
        else:
            print(f"✗ Joint end-to-end training hypothesis NOT supported")
        
        # Overall conclusion
        if joint_vs_epsilon_kl_ratio < sae_vs_epsilon_kl_ratio:
            improvement = ((sae_vs_epsilon_kl_ratio - joint_vs_epsilon_kl_ratio) / sae_vs_epsilon_kl_ratio) * 100
            ce_improvement = ((sae_vs_epsilon_ce_ratio - joint_vs_epsilon_ce_ratio) / sae_vs_epsilon_ce_ratio) * 100
            print(f"🎯 JOINT TRAINING REDUCES PATHOLOGICAL ERRORS by {improvement:.1f}% (KL), {ce_improvement:.1f}% (CE)")
        ...
        if joint_vs_sae_kl_ratio < 1.0:
            joint_improvement_kl = (1 - joint_vs_sae_kl_ratio) * 100
            joint_improvement_ce = (1 - joint_vs_sae_ce_ratio) * 100
            print(f"🔬 JOINT vs SAE-ONLY IMPROVEMENT: {joint_improvement_kl:.1f}% (KL), {joint_improvement_ce:.1f}% (CE)")
        ...
        return {
                    'kl_summary_stats': kl_summary_stats.to_dict(),
                    'ce_summary_stats': ce_summary_stats.to_dict(),
                    'kl_ratios': {
                        'sae_vs_epsilon': sae_vs_epsilon_kl_ratio,
                        'joint_vs_epsilon': joint_vs_epsilon_kl_ratio,
                        'joint_vs_sae': joint_vs_sae_kl_ratio
                    },
                    'ce_ratios': {
                        'sae_vs_epsilon': sae_vs_epsilon_ce_ratio,
                        'joint_vs_epsilon': joint_vs_epsilon_ce_ratio,
                        'joint_vs_sae': joint_vs_sae_ce_ratio
                    },
                    'kl_statistical_tests': {
                        'sae_vs_epsilon': {'statistic': sae_vs_epsilon_kl_stat, 'p_value': sae_vs_epsilon_kl_p},
                        'joint_vs_epsilon': {'statistic': joint_vs_epsilon_kl_stat, 'p_value': joint_vs_epsilon_kl_p},
                        'joint_vs_sae': {'statistic': joint_vs_sae_kl_stat, 'p_value': joint_vs_sae_kl_p}
                    },
                    'ce_statistical_tests': {
                        'sae_vs_epsilon': {'statistic': sae_vs_epsilon_ce_stat, 'p_value': sae_vs_epsilon_ce_p},
                        'joint_vs_epsilon': {'statistic': joint_vs_epsilon_ce_stat, 'p_value': joint_vs_epsilon_ce_p},
                        'joint_vs_sae': {'statistic': joint_vs_sae_ce_stat, 'p_value': joint_vs_sae_ce_p}
                    },
                    'conclusions': {
                        'gurnee_pathological_confirmed': sae_vs_epsilon_kl_ratio > 1.5 and sae_vs_epsilon_kl_p < 0.05,
                        'joint_training_supported': joint_vs_sae_kl_ratio < 0.8 and joint_vs_sae_kl_p < 0.05,
                        'joint_reduces_pathological': joint_vs_epsilon_kl_ratio < sae_vs_epsilon_kl_ratio,
...
def create_joint_kl_comparison_plots(results_df, analysis_results, output_dir):
    ...
    ratios = analysis_results['kl_ratios']
    conclusions = analysis_results['conclusions']
    ax.text(0.02, 0.98, f"KL Ratios:\nSAE vs ε-random: {ratios['sae_vs_epsilon']:.2f}x\n"
                         f"Joint vs ε-random: {ratios['joint_vs_epsilon']:.2f}x\n"
                         f"Joint vs SAE: {ratios['joint_vs_sae']:.2f}x\n\n"
                         f"CE Ratios:\nSAE vs ε-random: {analysis_results['ce_ratios']['sae_vs_epsilon']:.2f}x\n"
    ...
    position_kl_stats = results_df[results_df['position'] <= max_pos].groupby(['intervention_type', 'position'])['kl_divergence'].mean().unstack(0)
    position_ce_stats = results_df[results_df['position'] <= max_pos].groupby(['intervention_type', 'position'])['cross_entropy'].mean().unstack(0)
    
    # KL Divergence by position
    ...
    plt.tight_layout()
    position_path = plots_dir / "joint_kl_ce_by_position.png"
    ...
    kl_improvement_data = [
        ('SAE vs Random', kl_ratios['sae_vs_epsilon'], 'lightcoral'),
        ('Joint vs Random', kl_ratios['joint_vs_epsilon'], 'lightgreen'),
        ('Joint vs SAE', kl_ratios['joint_vs_sae'], 'lightyellow')
    ]
    ...
    # Cross-Entropy ratios
    ce_ratios = analysis_results['ce_ratios']
    ce_improvement_data = [
        ('SAE vs Random', ce_ratios['sae_vs_epsilon'], 'lightcoral'),
        ('Joint vs Random', ce_ratios['joint_vs_epsilon'], 'lightgreen'),
        ('Joint vs SAE', ce_ratios['joint_vs_sae'], 'lightyellow')
    ]
    ...
    plt.tight_layout()
    improvement_path = plots_dir / "joint_kl_ce_improvement_analysis.png"
...
def main():
    ...
    print(f"  • SAE vs ε-random ratio: {ratios['sae_vs_epsilon']:.3f}x (KL), {ce_ratios['sae_vs_epsilon']:.3f}x (CE)")
    print(f"  • Joint vs ε-random ratio: {ratios['joint_vs_epsilon']:.3f}x (KL), {ce_ratios['joint_vs_epsilon']:.3f}x (CE)")
    print(f"  • Joint vs SAE ratio: {ratios['joint_vs_sae']:.3f}x (KL), {ce_ratios['joint_vs_sae']:.3f}x (CE)")
    ...
    if conclusions['joint_reduces_pathological']:
        improvement_kl = ((ratios['sae_vs_epsilon'] - ratios['joint_vs_epsilon']) / ratios['sae_vs_epsilon']) * 100
        improvement_ce = ((ce_ratios['sae_vs_epsilon'] - ce_ratios['joint_vs_epsilon']) / ce_ratios['sae_vs_epsilon']) * 100
        joint_improvement_kl = (1 - ratios['joint_vs_sae']) * 100
        joint_improvement_ce = (1 - ce_ratios['joint_vs_sae']) * 100
        print(f"\n🎯 MAIN CONCLUSION: Joint end-to-end training reduces pathological errors")
        print(f"   KL improvement: {improvement_kl:.1f}% vs random, {joint_improvement_kl:.1f}% vs SAE-only")
        print(f"   CE improvement: {improvement_ce:.1f}% vs random, {joint_improvement_ce:.1f}% vs SAE-only")
    ...
    print(f"\nCOMPARISON ANALYSIS:")
    print(f"  🔬 Joint training effectiveness (KL): {(1-ratios['joint_vs_sae'])*100:.1f}% improvement over SAE-only")
    print(f"  🔬 Joint training effectiveness (CE): {(1-ce_ratios['joint_vs_sae'])*100:.1f}% improvement over SAE-only")
    print(f"  📊 Pathological error reduction (KL): {((ratios['sae_vs_epsilon']-ratios['joint_vs_epsilon'])/ratios['sae_vs_epsilon']*100):.1f}%")
    print(f"  📊 Pathological error reduction (CE): {((ce_ratios['sae_vs_epsilon']-ce_ratios['joint_vs_epsilon'])/ce_ratios['sae_vs_epsilon']*100):.1f}%")
