...
#USE THIS INSTEAD OF PART1/2 -- part1 was 18% sparsity, 66% var explained, 0.34 recon loss, lots of dead neurons
#
# THIS one 'Top K',
# 0.8642 variance explained, 2.05% active features, 0.1363 reconstruction loss
# 0% dead neurons 
# I stopped it ~ 200k training runs for brevity
#
# Parameters below: basically Top K =1024 (tried smaller, didn't work as well), bigger learning rate, zscore norming
# Kaiming initialization for encoder; 
# #Things NOT tried: positive bias on init; dead neuron tracking and recovery; 'batch topk'
...
K = 1024  # TopK parameter - only keep top 128 features per sample
TARGET_LAYER = 16  # LLaMA 3B has 32 layers, we target the middle layer
TRAIN_STEPS = 500_000
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
CHECKPOINT_DIR = "checkpoints_topk"
...
class TopKSparseAutoencoder(nn.Module):
    ...
    def initialize_weights(self):
        # Use your proven initialization
...
def train_sae(model, train_loader, val_loader, num_steps, checkpoint_dir=CHECKPOINT_DIR):
    """Train the TopK sparse autoencoder."""
    ...
    try:
        ...
        for step in tqdm(range(num_steps), desc="Training TopK SAE"):
            ...
            if step % VALIDATION_INTERVAL == 0:
                ...
                val_metrics = validate_sae(model, val_loader)
            ...
            if step % 1000 == 0:
                ...
                print(f"Train - % Active Features: {percent_active:.2f}% (Target: {K/NUM_FEATURES*100:.2f}%)")
...
def validate_sae(model, val_loader):
    """
    Compute validation metrics for the TopK SAE
...
def main():
    ...
    # Initialize and train TopK SAE
    print(f"Initializing TopK SAE with K={K}...")
    print(f"Target sparsity: {K}/{NUM_FEATURES} = {K/NUM_FEATURES*100:.2f}% active features")
    ...
    debug_model_parameters(sae, "initial_topk_sae")
    ...
    # Save the final trained TopK SAE
    torch.save(sae.state_dict(), "sae_topk_model.pt")
    print("Training complete! TopK SAE model saved as sae_topk_model.pt")
