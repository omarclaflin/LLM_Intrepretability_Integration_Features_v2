...
class TopKSparseAutoencoder(torch.nn.Module):
    """TopK Sparse Autoencoder module - matches the one from part2d."""
...
class StimulusResponseAnalyzer:
    ...
    def __init__(self, sae_model, nfm_model, tokenizer, base_model, device="cuda", target_layer=16, activation_method="mean"):
        ...
        self.activation_method = activation_method  # Add this line
    ...
    def discover_top_features(self, stimulus_sets, n_features=5, batch_size=16):
        ...
        for i in tqdm(range(0, len(all_texts), batch_size), desc="Processing texts for feature discovery"):
            ...
            # Also add this debugging version to your discover_top_features method
            # Replace the processing section with this heavily debugged version:

            with torch.no_grad():
                outputs = self.base_model(**inputs, output_hidden_states=True)
                hidden_states = outputs.hidden_states[self.target_layer]
                
                # Only debug first batch to reduce spam
                if i == 0:
                    print(f"\n=== DEBUG INFO FOR BATCH {i} ===")
                    print(f"Original hidden_states shape: {hidden_states.shape}")
                    print(f"Hidden states min/max: {hidden_states.min().item():.6f} / {hidden_states.max().item():.6f}")
                
                # Reshape hidden states from [batch, seq_len, hidden_dim] to [batch*seq_len, hidden_dim]
                batch_size, seq_len, hidden_dim = hidden_states.shape
                hidden_states_reshaped = hidden_states.reshape(-1, hidden_dim)
                
                features, _ = self.sae_model(hidden_states_reshaped.to(self.sae_model.encoder[0].weight.dtype))
                
                if i == 0:
                    print(f"SAE features shape: {features.shape}")
                    print(f"Number of non-zero features: {(features > 0).sum().item()}")
                
                # Reshape features back to [batch, seq_len, num_features]
                ...
                for b in range(features.shape[0]):
                    ...
                    if seq_len_actual > 0:
                        if self.activation_method == "max":
                            activation = torch.max(features[b, :seq_len_actual, :], dim=0)[0].cpu().numpy()
                        else:  # mean
                            activation = torch.mean(features[b, :seq_len_actual, :], dim=0).cpu().numpy()
                ...
                for b in range(features.shape[0]):
                    ...
                    if seq_len_actual > 0:
                        for feature_idx in feature_indices:
                            if self.activation_method == "max":
                                activation_value = torch.max(features[b, :seq_len_actual, feature_idx]).item()
                            else:  # mean
                                activation_value = torch.mean(features[b, :seq_len_actual, feature_idx]).item()
                            feature_activations[feature_idx].append(activation_value)
...
def test_sae_functionality(sae_model, device):
    """Test if the SAE is working with random input."""
    print("\n=== TESTING SAE FUNCTIONALITY ===")
    
    # Create random input matching expected dimensions
    batch_size = 4
    input_dim = sae_model.encoder[0].in_features
    test_input = torch.randn(batch_size, input_dim, device=device, dtype=sae_model.encoder[0].weight.dtype)
    
    print(f"Test input shape: {test_input.shape}")
    print(f"Test input dtype: {test_input.dtype}")
    print(f"Test input min/max: {test_input.min().item():.6f} / {test_input.max().item():.6f}")
    
    with torch.no_grad():
        features, reconstruction = sae_model(test_input)
        
        print(f"SAE features shape: {features.shape}")
        print(f"SAE features min/max: {features.min().item():.6f} / {features.max().item():.6f}")
        print(f"Non-zero features per sample: {(features > 0).sum(dim=1)}")
        print(f"Expected k per sample: {sae_model.k}")
        print(f"Total non-zero features: {(features > 0).sum().item()}")
        
        # Check if TopK is working
        for i in range(min(2, batch_size)):
            sample_features = features[i]
            nonzero_count = (sample_features > 0).sum().item()
            print(f"Sample {i}: {nonzero_count} non-zero features (should be {sae_model.k})")
            if nonzero_count > 0:
                print(f"Sample {i} top 5 values: {torch.topk(sample_features, min(5, nonzero_count)).values}")
...
def main():
    ...
    parser.add_argument("--sae_k", type=int, default=500, help="K value for TopK SAE (default: 500)")
    ...
    # Load TopK SAE model - FIXED SECTION
    sae_checkpoint = torch.load(args.sae_path, map_location=args.device)

    # Handle different checkpoint formats
    if 'model_state' in sae_checkpoint:
        sae_state_dict = sae_checkpoint['model_state']
        print("Found 'model_state' key in checkpoint")
    else:
        sae_state_dict = sae_checkpoint
        print("Using checkpoint directly as state_dict")

    print("SAE state_dict keys:", list(sae_state_dict.keys()))
    ...
    if 'decoder.weight' in sae_state_dict:
        ...
        print(f"Found decoder.weight with shape: {sae_state_dict['decoder.weight'].shape}")
    elif 'encoder.0.weight' in sae_state_dict:
        encoder_weight = sae_state_dict['encoder.0.weight']
        hidden_dim, input_dim = encoder_weight.shape
        print(f"Found encoder.0.weight with shape: {encoder_weight.shape}")
    else:
        print("ERROR: Could not find decoder.weight or encoder.0.weight in state_dict!")
        print("Available keys:", list(sae_state_dict.keys()))
        raise KeyError("Could not determine SAE dimensions from checkpoint")

    print(f"Inferred dimensions: input_dim={input_dim}, hidden_dim={hidden_dim}")

    # Create TopK SAE with specified k value
    sae_model = TopKSparseAutoencoder(input_dim, hidden_dim, args.sae_k)
    sae_model.load_state_dict(sae_state_dict)
    sae_model.to(args.device)

    print(f"Loaded TopK SAE with input_dim={input_dim}, hidden_dim={hidden_dim}, k={args.sae_k}")

    # TEST THE SAE FUNCTIONALITY
    test_sae_functionality(sae_model, args.device)
    # ADD THIS LINE:
    test_sae_functionality(sae_model, args.device)
    ...
    # Initialize analyzer
    analyzer = StimulusResponseAnalyzer(sae_model, nfm_model, tokenizer, base_model, args.device, activation_method=args.activation_method)
